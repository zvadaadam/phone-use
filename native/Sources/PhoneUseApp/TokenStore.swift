...
final class TokenStore: @unchecked Sendable {
    ...
    init(baseURL: URL? = nil) throws {
        ...
        try Self.validateOwnedItem(at: directory, expectedType: .typeDirectory)
        try fileManager.setAttributes([.posixPermissions: 0o700], ofItemAtPath: directory.path)

        fileURL = directory.appendingPathComponent("token", isDirectory: false)
        if fileManager.fileExists(atPath: fileURL.path) {
            try Self.validateOwnedItem(at: fileURL, expectedType: .typeRegular)
    ...
    func exchangeDashboardBootstrap(_ value: String) throws -> String? {
        ...
        guard let expiry = bootstraps.removeValue(forKey: value), expiry > now else {
            return nil
        }
    ...
    private static func validateOwnedItem(
        ...
        guard attributes[.type] as? FileAttributeType == expectedType else {
            throw TokenError("Security-sensitive path has an unexpected type")
        }
